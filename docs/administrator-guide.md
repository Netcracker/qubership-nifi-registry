...
| NIFI_REGISTRY_DB_DEBUG_SQL                       | N                                 | false   | Whether or not enable debug logging for SQL statements. Defines value for `nifi.registry.db.maxConnections` property.                                                                                                                                  |
| NIFI_REGISTRY_SECURITY_USER_OIDC_CONNECT_TIMEOUT | N                                 |         | Socket Connect timeout when communicating with the OpenID Connect Provider. Defines value for `nifi.registry.security.user.oidc.connect.timeout` property.                                                                                             |
| NIFI_REGISTRY_SECURITY_USER_OIDC_READ_TIMEOUT    | N                                 |         | Socket Read timeout when communicating with the OpenID Connect Provider. Defines value for `nifi.registry.security.user.oidc.read.timeout` property.                                                                                                   |

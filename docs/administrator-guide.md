...
| NIFI_REG_DB_PASSWORD                             | Y (if NIFI_REG_USE_PGDB = true)   |         | Defines password for DB connection. May contain any printable characters except for `\` (backslash).                                                                                                                                                   |
...
| NIFI_CONSUL_INT_FRAMEWORK                        | N                                 | quarkus | Integration framework to use for connecting to Consul. One of: `spring`, `quarkus`. Default is `quarkus`. `quarkus` version requires less resources and starts faster.                                                                                 |
| CONSUL_URL                                       | Y (if CONSUL_ENABLED = true)      |         | URL to access Consul service. If `NIFI_CONSUL_INT_FRAMEWORK` = `spring`, it must be in format: `<hostname>:<port>`. Otherwise, `<protocol>://<hostname>:<port>`.                                                                                       |
